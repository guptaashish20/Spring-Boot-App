...
public class JournalService {
	...
	public void insertData() {
		...
		/* Login to H2 Database on this url "http://localhost:8080/h2-console" */
		String dropTableQuery = "DROP TABLE JOURNAL IF EXISTS";
		String createTableQuery = "CREATE TABLE JOURNAL (id SERIAL, title VARCHAR(255), summary VARCHAR(255), created TIMESTAMP)";
	...
	public List<Journal> findAll(){
        ...
        jdbcTemplate.query("SELECT * FROM JOURNAL",	new Object[]{},	(rs,row) -> new Journal(rs.getLong("id"), rs.getString("title"), rs.getString("summary"),
                                					new Date(rs.getTimestamp("created").getTime()))).forEach(entry -> entries.add(entry));
        return entries;
	}   
}
